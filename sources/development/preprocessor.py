...
class Preprocessor:
    ...
    def analyse_dataframe(self, df: pd.DataFrame, target:str = None):
        ...
        print(f'% of trips within 1-60 min: {((df[target] >= 60) & (df[target] <= 60*60)).mean()*100:.2f}')
        
        return numerical_columns, categorical_columns

    def preprocess_data (self, df:pd.DataFrame, categorical_features:List[str], numerical_features:List[str], target:str=''):
        """
    ...
    def prepare_dictionaries(self, df:pd.DataFrame, categorical_features:List[str], numerical_features:List[str]):

        dicts = df[categorical_features + numerical_features].to_dict(orient='records')
        return dicts

    def prepare_features(self, dv:DictVectorizer, dict, y:np.ndarray, fit:bool = True):
...
if __name__ == "__main__":
    ...
    dv = DictVectorizer()
    X, y, dv = preprocessor.process(TRAIN_PATHFILE,DATE_FIELDS,TARGET,CATEGORICAL_FEATURES,dv=dv,dv_fit=True)
