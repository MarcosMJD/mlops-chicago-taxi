...
def preprocess_datasets(TRAIN_PATHFILE: str, VAL_PATHFILE: str, TEST_PATHFILE: str):
    ...
    X_train, y_train, dv, X_train_dicts = data_processor.process(
        TRAIN_PATHFILE, DATE_FIELDS, TARGET, CATEGORICAL_FEATURES, dv, True
    )
    X_val, y_val, dv, X_val_dicts = data_processor.process(
        VAL_PATHFILE, DATE_FIELDS, TARGET, CATEGORICAL_FEATURES, dv, False
    )
    X_test, y_test, dv, X_test_dicts = data_processor.process(
        TEST_PATHFILE, DATE_FIELDS, TARGET, CATEGORICAL_FEATURES, dv, False
    )
    return (
        X_train,
        X_train_dicts,
...
def train_models(
    ...
    for i, model_class in enumerate(model_classes):
        ...
        with mlflow.start_run():
            ...
            print(f"Sample number 50: {json.dumps(X_train_dicts[50])} => {y_pred[50]}")
            rmse = mean_squared_error(y_val, y_pred, squared=False)
