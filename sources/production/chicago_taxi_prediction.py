...
MODEL_LOCATION = os.getenv("MLFLOW_MODEL_LOCATION", "./model")

# Todo: get model location from mlflow's model registry server
if MODEL_LOCATION == "s3":
    model = model_service.init_model_s3(S3_BUCKET_NAME, S3_BUCKET_FOLDER, EXPERIMENT_ID, RUN_ID)
else:
    model = model_service.init_model_local(MODEL_LOCATION)
